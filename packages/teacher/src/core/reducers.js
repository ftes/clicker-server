...
const coreReducer = (state = {}, action) => {
  const stateAfterSave = saveReducer(state, action, preserveOnReset);
  const stateAfterSyncLogicBefore = syncLogicBefore(stateAfterSave, action);
  const reducedState = reducers(stateAfterSyncLogicBefore, action);
  const stateAfterSyncLogicAfter =
    syncLogicAfter(reducedState, action, deleteOnSync, actionTypesToSkipOnSync);
  return stateAfterSyncLogicAfter;
};
